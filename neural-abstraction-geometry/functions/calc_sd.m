function [sd,performance,parsedParams] = calc_sd(T,condLabels,nv)
...
% significance for each performance metric) on all dichotomies.
%
% PARAMETERS
% ----------
...
%                       of T overall decreases by n) before calculating
%                       CCGP.
...
%                     positive for each class, divided by the total number
%                     of observations; this is also equivalent to
%                     micro-averaged precision, recall, and f-measure),
%                     averaged across repetitions, for the i_th
%                     dichotomy/dataset.
%   .balaccuracy    : nDichotomies x 1 vector whose i_th element is the the
%                     balanced accuracy across classes (average of recall
%                     for each class), averaged across repetitions, for the
%                     i_th dichotomy/dataset.
%   .precision      : nDichotomies x nClasses matrix whose i_th j_th
%                     element is the precision (number of true positives
%                     divided by number of predicted positives) for the
%                     j_th class as positive, averaged across repetitions,
%                     in the i_th dichotomy.
%   .recall         : nDichotomies x nClasses matrix whose i_th j_th
%                     element is the precision (number of true positives
%                     divided by number of actual positives) for the j_th
%                     class as positive, averaged across repetitions, in
%                     the i_th dichotomy.
%   .fmeasure       : nDichotomies x nClasses matrix of f-measure scores
%                     whose i_th j_th element is the f-measure for the j_th
%                     class as positive, averaged across repetitions, in
%                     the i_th dichotomy.
%   .aucroc         : nDichotomies x nClasses matrix of AUC ROC values
%                     whose i_th j_th element is the AUC ROC for the j_th
%                     class as positive, averaged across repetitions, in
%                     the i_th dichotomy.
...
%                     class, columns: predicted class) for the j_th
%                     repetition in the i_th dichotomy.
%   .labels         : Optionally returned field (if 'saveLabels' = true and
%                     'oversampleTest' = false) consisting of an
%                     nDichotomies x nReps x nObservations numeric array,
%                     where the i_th row contains the predicted labels
%                     (aggregated across folds) for the i_th repetition.
%   .scores         : Optionally returned field (if 'saveScores' = true and
%                     'oversampleTest' = false) consisting of an
%                     nDichotomies x nReps x nObservations x nClasses
%                     numeric array, where the (i,j,:,:) slice contains the
%                     nObservations x nClasses matrix for the j_th
%                     repetition in the i_th dichotomy, and the k_th l_th
%                     element (of this slice) is the score for the k_th
%                     observation being classified into the l_th class.
%   .sig            : Scalar struct with the following fields (all
%                     individual null performance values, from whose
%                     aggregate the following measures are calculated are
%                     generated from only one repetition of a given
%                     permutation).
%       .p       : Scalar struct with the following fields (note: these p
%                  values are calculated using an observed performance
%                  value that is the average across repetitions).
...
%                          (i,:,:) slice is a matrix whose j_th row has as
%                          its 1st and 2nd elements the lower and upper
%                          interval bounds on the recall null distribution,
%                          where the j_th class is positive, in the i_th
%                          dichotomy.
...
%                          (i,:,:) slice is a matrix whose j_th row has as
%                          its 1st and 2nd elements the lower and upper
%                          interval bounds on the AUC PR null distribution,
%                          where the j_th class is positive, in the i_th
%                          dichotomy.
...
%                     will be absent from decoderPerf.
...
if isempty(nv.decoderParams), nv.decoderParams = struct('a', []); end
parsedParams = parse_inputs(nv.decoderParams);
    
% Test decoding on same dataset but with different class labels (for
% different dichotomies).
performance ...
    = multiset_decoder(T, dichotLabels, ...
