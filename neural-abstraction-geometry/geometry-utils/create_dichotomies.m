function [dichotomies,dichotomyConds] = create_dichotomies(nConds,condNames)
% Accepts as input nConds, the number of conditions, and an optional
% argument condNames, a 1 x nConds cell array containing the names of each
...
% condNames  : 1 x nConds cell array, where each cell contains a string
%              which is the name of a condition (and that condition's index
%              is the same as the index of its name in condNames). This
%              argument is optional, and the user may also supply an empty
%              array, in which case dichotomyConds will be returned as an
%              empty array as well.
%
% RETURNS
% -------
...
%                  row elements being the indices of conditions on one side
%                  of the dichotomy, and the latter half the indices of the
%                  conditions on the other side of the dichotomy.
...
%                  condNames was empty, this variable will also be returned
%                  as an empty array.
%                  
% Author: Jonathan Chien 7/20/21. Last edit: 7/20/21.
...
if ~isempty(condNames)
    dichotomyConds(:,1:m) = condNames(dichotomies(:,1:m));
    dichotomyConds(:,m+1:nConds) = condNames(dichotomies(:,m+1:nConds));
else
    dichotomyConds = [];
end
