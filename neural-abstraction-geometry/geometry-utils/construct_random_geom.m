function randGeom = construct_random_geom(T,nConds,nvp)
% Accepts as input a t x n matrix where t = q*p, with q being the number of
% conditions, p the number of trials per condition, and n the number of
% neurons. p x n is thus a point cloud around each empirical cluster
% centroid. Output is also a t x n matrix, however this t x n matrix
% consists of C random cluster centroids (sampled from a standard gaussian
...
%   'addTrials' : This name value pair takes on a value of logical true or
...
% RETURNS
% -------
% randGeom : Either a t x n matrix (with t = nTrials and n = nNeurons) or a
%            q x n matrix (with q = nConds and n = nNeurons), depending on
%            the value of the 'addTrials' name value pair (see PARAMETERS
%            above). If t x n, the i_th row j_th column element is the
%            firing rate of the j_th neuron on the j_th single trial. If q
%            x n, the i_th row j_th column element is the firing rate of
%            the i_th neuron averaged over all trials of the j_th
%            condition.
%
...
if nvp.addTrials
    % For each condition, take mean centered point cloud (p x n) from
    ...
    for iCond = 1:nConds
        ...
        cloud = (T((iCond-1)*nTrialsPerCond + 1 : iCond*nTrialsPerCond, :));

        % Obtain mean centered point cloud.
        meanCenteredCloud = cloud - mean(cloud);

        % Perform rotation by permuting neurons.
        rotMeanCenteredCloud = meanCenteredCloud(:,randperm(nNeurons));
        ...
        randomCluster = randomCentroids(iCond,:) + rotMeanCenteredCloud;
