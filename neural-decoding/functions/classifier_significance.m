...
% ground_truth : nObservations x 1 vector of integer class labels.
% Name-Value Pairs (nv)
...
%   'condLabels'       : ([] | nObservations x 1 vector), if vector, must
%                        be in one-to-one correspondence to both the rows
%                        of X and the elements of ground_truth. The i_th
%                        element is the condition index of the i_th
%                        observation/trial.
...
%                  positives divided by number of predicted positives,
%                  where the i_th element defines positive as membership in
%                  the i_th class), averaged across repetitions.
...
%                  consisting of an nCvReps x nClasses x nClasses array
%                  where the (i,:,:) slice contains the nClasses x nClasses
...
%                  'oversampleTest' = false) consisting of an nCvReps x
...
%           .balaccuracy : p value for balanced accuracy.
...
%           .accuracy    : 2-vector (row) whose 1st and 2nd elements are
%                          the lower and upper bounds of the interval on
%                          the accuracy null distribution.
%           .balaccuracy : 2-vector (row) whose 1st and 2nd elements are
%                          the lower and upper bounds of the interval on
%                          the balanced accuracy null distribution.
...
%                          distribution that the observed accuracy
%                          (averaged across repetitions) lies.
%           .balaccuracy : Scalar value that is the number of standard
%                          deviations from the mean of the null balanced
%                          accuracy distribution that the observed balanced
%                          accuracy (averaged across repetitions) lies.
...
if nvp.cvFun == 1
    [performance, labels, scores] ...
        = crossval_model_1(X, groundTruth, ...
                           ...
                           'classCoding', nvp.classCoding, 'nCvReps', nvp.nCvReps, ...
    ...
    if nvp.cvFun == 1
        [nullMetrics{iPerm},~,~] ...
            = crossval_model_1(nullX, nullGroundTruth, ...
                               'classifier', nvp.classifier, ...
                               'classCoding', nvp.classCoding, 'nCvReps', 1, ...
                               'kFold', nvp.kFold, 'useGpu', nvp.useGpu);
    elseif nvp.cvFun == 2
        [nullMetrics{iPerm},~,~] ...
...
for iMetric = 1:length(metricNames)
    ...
    performance.sig.nullInt.(currMetric) ...
        = interval_bounds(nullPerf.(currMetric), nvp.nullInt, NULL_DIM);
    performance.sig.zscore.(currMetric) ...
        = null_zscore(performance.(currMetric), nullPerf.(currMetric), NULL_DIM);
end
...
% As of this writing, MATLAB treats an indexing request into a.x as an
...
% field is accuracy/balaccuracy, in which case the 1st dim size is 1), 2nd
% dim size = nPerms).
