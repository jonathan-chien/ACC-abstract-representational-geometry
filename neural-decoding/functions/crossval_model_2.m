...
%                       elements of groundTruth. The i_th element is the
...
%          nCvReps x 1 cell array, where each cell contains a row vector of
...
%          test fold (if a condition or class is quite rare). Thus, storage
%          in a cell array is necessary.
...
%          class. If 'oversampleTest' evaluates to true, scores is returned
%          as an nCvReps x 1 cell array, where the i_th cell contains the
%          nTestingLabels x nClasses matrix of scores for the i_th
%          repetition (cell array used for the same reason as with labels
%          above).
%
...
% Check number of classes. If > 2, use fitcecoc and notify user of this if
% another function handle was passed in.
...
% matrices, labels, and scores across repetitions. 
nObs = length(groundTruth);
confusionMats = NaN(nvp.nCvReps, nClasses, nClasses);
labels = cell(nvp.nCvReps, 1);
scores = cell(nvp.nCvReps, 1);
allTestLabels = cell(nvp.nCvReps, 1);
...
% Squeeze in case there was only one rep.
...
if range(nTrialsByClass) > 0 && BALANCE
