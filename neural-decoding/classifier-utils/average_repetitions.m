...
%   .balaccuracy -- Scalar value, balanced accuracy (average of recall for
%                   each class), averaged across repetitions.
%   .precision   -- nClasses x 1 vector of precision values, averaged
%                   across repetitions, where the i_th element has the i_th
%                   class as positive.
%   .recall      -- nClasses x 1 vector of recall values, averaged
%                   across repetitions, where the i_th element has the i_th
%                   class as positive.
%   .fmeasure    -- nClasses x 1 vector of f-measure values, averaged
%                   across repetitions, where the i_th element has the i_th
%                   class as positive.
%   .aucroc      -- nClasses x 1 vector of AUC ROC values, averaged
%                   across repetitions, where the i_th element has the i_th
%                   class as positive.
%   .aucpr       -- nClasses x 1 vector of AUC PR values, averaged
%                   across repetitions, where the i_th element has the i_th
%                   class as positive.
%
...
for iRep = 1:nReps
    ...
    % Accuracy.
    [metrics.accuracy(iRep), metrics.balaccuracy(iRep)] = calc_accuracy(current);
    ...
    [metrics.aucroc(:,iRep), metrics.aucpr(:,iRep)] ...
        = calc_auc(groundTruth{iRep}, scores{iRep}, 'handleNaN', true);
end
